...
(deftest literal-test
  (let [frontend (->MockFrontend)]
    (testing "integer"
      (let [root (m/->Literal 42 nil nil nil)
            results (infer/infer [root] :frontend frontend)
            node (first results)]
        (is (tcon? (get-type node) :int64))))
    (testing "float"
      (let [root (m/->Literal 3.14 nil nil nil)
            results (infer/infer [root] :frontend frontend)
            node (first results)]
        (is (tcon? (get-type node) :float64))))
    (testing "string"
      (let [root (m/->Literal "hi" nil nil nil)
            results (infer/infer [root] :frontend frontend)
            node (first results)]
        (is (tcon? (get-type node) :string))))
    (testing "boolean"
      (let [root (m/->Literal true nil nil nil)
            results (infer/infer [root] :frontend frontend)
            node (first results)]
...
(deftest variable-test
  (let [frontend (->MockFrontend)]
    ...
    (testing "unbound variable"
      (let [node (m/->Variable "y" nil nil nil)
  ...
  (let [frontend (->MockFrontend)]
    (testing "successful call"
      (let [fn-ty (t/->TFun (t/->TCon :int64) (t/->TCon :int64))
            fn-node (m/->Variable "f" nil nil nil)
            arg-node (m/->Literal 1 nil nil nil)
            call-node (m/->Call fn-node [arg-node] nil nil nil)
            [ty result] (infer/local-infer call-node {:frontend frontend :env {"f" fn-ty}})]
        (is (tcon? ty :int64))
        (is (tcon? (get-type result) :int64))))
    (testing "unknown function"
      (let [fn-node (m/->Variable "g" nil nil nil)
            arg-node (m/->Literal 1 nil nil nil)
            call-node (m/->Call fn-node [arg-node] nil nil nil)
...
(deftest lambda-test
  (let [frontend (->MockFrontend)
        param-node (m/->Variable "x" nil nil nil)
        body-node (m/->Variable "x" nil nil nil)
...
(deftest while-test
  (let [frontend (->MockFrontend)]
    (testing "simple while loop"
      (let [test-node (m/->Literal true nil nil nil)   ;; 条件恒为真
            body-node (m/->Literal 1 nil nil nil)      ;; 体返回整数
    ...
    (testing "while with unknown body"
      (let [test-node (m/->Literal true nil nil nil)
            body-node (m/->Variable "x" nil nil nil)   ;; x 未绑定
            while-node (m/->While test-node body-node nil nil nil)
            [ty result] (infer/local-infer while-node {:frontend frontend :env {}})]
        (is (nil? ty))
        (is (nil? (get-type result)))))))