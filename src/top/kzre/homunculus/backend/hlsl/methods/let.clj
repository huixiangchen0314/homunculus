(ns top.kzre.homunculus.backend.hlsl.methods.let
  ...
  (:require [top.kzre.homunculus.backend.hlsl.core :as core]
            [top.kzre.homunculus.core.ir2.node :as n]         ; 提供 let-bindings, let-body, attrs, node-meta 等
...
(defmethod core/emit-node :let [node context]
  (let [bindings (n/let-bindings node)   ;; 现在是 Binding 向量
        ...
        decls (mapv (fn [b]
                      (let [v (:var b)           ; Variable 节点
                            e (:val b)           ; 表达式节点
                            ir-type (ty/get-type v)]
                        (if (ty/vec-type? ir-type)
                          [:expr-stmt
                           [:array-decl
                            (core/hlsl-type-str (ty/vec-element-type ir-type))
                            (cname (:name v))             ; 直接从 Variable 节点取名称
